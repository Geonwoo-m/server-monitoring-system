...
<summary>📂 프로젝트 초기 기록 (2025/12/16 ~ 12/17) 보기/접기</summary>
< 2025/12/16 시작 >
...
## 📝 주석처리  (2025-12-21)
 ...
 데이터 흐름을 명확히 파악하기 위해 일부 서버 로직에 대한 주석작업 

### 1. API 엔드포인트 로직 분석 (`/api/history`)
* **요청 파라미터 처리**: `Query String`을 파싱하여 동적으로 `range`와 `unit` 값을 설정하는 반복문 로직을 분석 및 기술했습니다.
* **JSON 데이터 직렬화(Serialization)**: 
  - DB의 리스트 데이터를 `JSONArray`로 변환하고, 각 요소를 `Map` 구조의 `JSONObject`로 구성하는 과정을 문서화했습니다.

   응답 데이터에 실시간 수신 상태 확인을 위한 `lastSeen` 필드를 추가하여 클라이언트의 시각화 효율을 높였습니다.
* **네트워크 전송 규격 준수**: 
  - **Header**: `Content-Type: application/json` 설정을 통해 응답 형식을 명시.
  - **Payload**: 바이트 배열(`byte[]`)로 변환된 실 데이터를 전송하는 과정을 정리.
  - **Resource**: 통신 종료 후 `close()`를 통한 스트림 자원 반납 과정을 정리.

### 💡 여기서 느낀점

>  **Header와 Payload**의 개념이 나오면서 네트워크 공부의 중요성을 다시 한 번 느끼게 됨.

### 2. 데이터베이스 접근 로직 분석 (`Repository`)

* **데이터 집계 및 그룹화**: 
  - `ResultSet`의 엑셀 형태 데이터를 자바 객체(`MetricSummary`)로 매핑하는 과정을 주석으로 정리.
* **데이터 정렬 최적화**: 
  - DB에서 최신순(`DESC`)으로 조회하는 것이 성능상 유리함을 확인하고, 그래프 시각화를 위해 자바에서 `Collections.reverse()`를 통해 시간순으로 재정렬하는 과정을 정리 

### 3. 예외 처리 및 방어 코드
* 데이터 부재 시 예외 상황 대신 `999`라는 약속된 상태 코드를 반환하여 시스템의 중단 없이 클라이언트가 상태를 인지할 수 있도록 설계된 로직을 문서화했습니다.
